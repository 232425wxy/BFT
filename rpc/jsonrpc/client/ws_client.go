...
func (c *WSClient) Send(ctx context.Context, request types.RPCRequest) error {
	select {
	case c.send <- request:
		c.Logger.Infow("sent a request", "req", request)
		// c.mtx.Lock()
...
func (c *WSClient) reconnect() error {
	...
	for {
		jitter := time.Duration(srrand.Float64() * float64(time.Second)) // 1s == (1e9 ns)
		backoffDuration := jitter + ((1 << uint(attempt)) * time.Second)

		c.Logger.Infow("reconnecting", "attempt", attempt+1, "backoff_duration", backoffDuration)
...
func (c *WSClient) processBacklog() error {
	...
	case request := <-c.backlog:
		...
		c.Logger.Infow("resend a request", "req", request)
	default:
	}
	return nil
}
...
func (c *WSClient) readRoutine() {
	...
	for {
		...
		c.Logger.Infow("got response", "id", response.ID, "result", fmt.Sprintf("%X", response.Result))
