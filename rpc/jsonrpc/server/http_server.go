...
func Serve(listener net.Listener, handler http.Handler, logger log.CRLogger, config *Config) error {
...
// WriteRPCResponseHTTPError 将 res 封送为 JSON (带缩进)并写入 w，
// 只有在 res 中的 Error 字段不为 nil 的时候才会调用此方法
...
func RecoverAndLogHandler(handler http.Handler, logger log.CRLogger) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		defer func() {
			...
			logger.Debugw("served RPC HTTP response",
				"method", r.Method,
				"url", r.URL,
				"status", rww.Status,
				"duration", durationMS,
