...
func MapToRequest(id jsonrpcid, method string, params map[string]interface{}) (RPCRequest, error) {
	...
	for name, value := range params {
		valueJSON, err := srjson.Marshal(value)
		if err != nil {
...
// From the JSON-RPC 2.0 spec:
//	如果在 Request 对象中检测 id 时出现错误(例如 Parse error/Invalid Request)
...
// RemoteAddr 返回远端地址(通常是字符串 "IP:port")，如果没有设置HTTPReq和WSConn，则返回一个空字符串 ""
// HTTP:
...
// Context 返回请求的上下文，返回的上下文总是非 nil;它默认为 context.Background()
// 	HTTP:
// 	    当客户端连接关闭，请求被取消(HTTP/2)，或者当 ServeHTTP 方法返回时，context 被取消
// 	WS:
//	    当客户端连接关闭时，context 被取消
