...
func (txi *TxIndex) Get(hash []byte) (*protoabci.TxResult, error) {
	if len(hash) == 0 {
	...
	txResult := new(protoabci.TxResult)
...
// Any event with an empty type is not indexed.
func (txi *TxIndex) Index(result *protoabci.TxResult) error {
...
func (txi *TxIndex) indexEvents(result *protoabci.TxResult, hash []byte, store dbm.Batch) error {
...
func (txi *TxIndex) Search(ctx context.Context, q *query.Query) ([]*protoabci.TxResult, error) {
	select {
	case <-ctx.Done():
		return make([]*protoabci.TxResult, 0), nil
	...
	if err != nil {
		...
		switch {
		case err != nil:
			return []*protoabci.TxResult{}, fmt.Errorf("error while retrieving the result: %w", err)
		case res == nil:
			return []*protoabci.TxResult{}, nil
		default:
			return []*protoabci.TxResult{res}, nil
		}
	}
	...
	results := make([]*protoabci.TxResult, 0, len(filteredHashes))
...
func keyForEvent(key string, value []byte, result *protoabci.TxResult) []byte {
...
func keyForHeight(result *protoabci.TxResult) []byte {
