...
func (pb *playback) replayReset(count int, newStepSub types.Subscription) error {
	...
	newCS := NewState(pb.cs.config, pb.genesisState.Copy(), pb.cs.blockExec,
		pb.cs.blockStore, pb.cs.txNotifier, nil)
	...
	for {
		...
		case "rs":
			...
			if len(tokens) == 1 {
				...
				case "proposal":
					fmt.Println(rs.PrePrepare)
				case "proposal_block":
					fmt.Printf("%v %v\n", rs.PrePrepareBlockParts.StringShort(), rs.PrePrepareBlock.StringShort())
				case "locked_round":
...
func newConsensusStateForReplay(config cfg.BaseConfig, csConfig *cfg.ConsensusConfig) *State {
	...
	blockExec := sm.NewBlockExecutor(stateStore, log.NewCRLogger("info"), proxyApp.Consensus(), mempool)

	consensusState := NewState(csConfig, state.Copy(), blockExec,
		blockStore, mempool, nil)
